<template>
  <nav class="navbar navbar-expand-lg navbar-dark">
    <div class="container-fluid">
      ...
      <a class="navbar-brand me-3 ms-3"><i class="fas fa-shopping-basket"></i> {{ title }} </a>
      ...
      <form class="d-flex">
          <a href="#" class="btn btn-dark me-3"><i class="fas fa-sign-in-alt"></i> Login</a>
          <div class="Select List">
            <select class="form-select form-select" @change="selectedList($event)">
              <option value="">Select a list</option>
              <option
                :key="index"
                v-for="(l, index) in getLists"
                :value="l.id"
              >
                {{ l.name }}
              </option>
